...
#let ex25-fig = cetz.canvas({
  ...
  for (i, t) in ts.enumerate() {
    circle(pt(t), radius: 0.07, fill: black, stroke: none)
    content(
      (1.22 * a * calc.cos(t), 1.22 * b * calc.sin(t)),
      text(size: 9pt)[$x_#(i + 1)$],
    )
  }

  // 領域ラベル
  content((0, 0), text(size: 10pt, fill: rgb("#2980b9"))[$P_"inner"$])
  content(
    (0.95 * a * calc.cos(340deg), 0.95 * b * calc.sin(340deg)),
    text(size: 10pt, fill: rgb("#2c3e50"))[$C$],
  )
  content(
    (1.1 * outer.at(0).at(0), 1.1 * outer.at(0).at(1)),
    text(size: 10pt, fill: rgb("#c0392b"))[$P_"outer"$],
  )
})

#figure(
  context if target() == "html" { html.frame(ex25-fig) } else { ex25-fig },
  caption: [$P_"inner" subset.eq C subset.eq P_"outer"$],
)
